...
public class sga {
	...
	public static void main(String[] args) throws Exception {
		...
		if(arg.equalsIgnoreCase("-h")) {
			System.out.println("\n\nA Simple Genetic Algorithm Script that builds the best generation using fitness \"onemax\" function. The steps performed are Selection, Recombination,"
					...
					+ "(duplicates do not matter) and the entire population\n"
					+ "\n"
					+ "-G : Full debugging will be displayed while running. For example, showing the "
...
//		File file = new File(absolute);
		...
		/*
		 * Termination condition - check if best and average fitness doesnot improve in the previous 3 generations
		 */
		while (!termination_flag) {
			...
			if(arg.equals("-G")) {
			System.out.println(
					"\nBEST FIT PARENT :" + best_fit_parent.getBinary_rep() + " -> " + best_fit_parent.getFitness_value());
			}
			...
			while (offSpring_population.size() <= populationSize) {
				...
				for (int i = 0; i < parents_selected.size(); i++) {
					System.out.println("Parent "+(i+1) +": " + parents_selected.get(i).getBinary_rep() + "-> Fitness: "
							+ parents_selected.get(i).getFitness_value());
				}
				}
				...
				List<String> children_after_recombination = Recombination.recombination(parents_selected,
						crossoverMethod, rand, probApplyCrossover);
				...
				for (int i = 0; i < evaluated_offsprings.size(); i++) {
					System.out.println("Child " + (i+1) + ": " + evaluated_offsprings.get(i).getBinary_rep() + " -> Fitness: "
							+ evaluated_offsprings.get(i).getFitness_value());
				}
				}
				...
				for (int i = 0; i < offSpring_population.size(); i++) {
					System.out.println("Child " + (i+1) + ": " + offSpring_population.get(i).getBinary_rep() + " -> Fitness: "
							+ offSpring_population.get(i).getFitness_value());
				}
				}
			 ...
			 */
			List<Candidate> final_population_after_replacemnt = Replacement
					.replacement_of_parents_with_offsprings(offSpring_population, best_fit_parent, replacementMethod)
					.getFinal_populatio_after_replacemnt();
			...
			int best = Replacement.replacement_of_parents_with_offsprings(offSpring_population, best_fit_parent,replacementMethod)
					.getBest_fit();
			int avg = (int) Replacement.replacement_of_parents_with_offsprings(offSpring_population, best_fit_parent,replacementMethod)
					.getAvg_fit();
			int worst = Replacement.replacement_of_parents_with_offsprings(offSpring_population, best_fit_parent,replacementMethod)
					.getWorst_fit();
			...
			//check if best and average fitness doesnot improve in the previous 3 generations
			if (previous_generations_bests.size() < 5) {
				...
				if (previous_generations_bests.size() == 4) {
					check_if_best_case_does_not_improve = check_if_best_case_does_not_improve(previous_generations_bests);
			...
			if (check_if_best_case_does_not_improve && check_if_avg_case_does_not_improve) {
				...
				System.out.println("\n\nBest String: "+Replacement.replacement_of_parents_with_offsprings(offSpring_population, best_fit_parent,replacementMethod).getBest_fit_string()+
						" Worst String : "+Replacement.replacement_of_parents_with_offsprings(offSpring_population, best_fit_parent,replacementMethod).getWorst_fit_string());
			...
			if (best == stringSize) {
				System.out.println("\n\nGlobal Fitness of " + best + " is achieved at location: "
						+ Replacement.replacement_of_parents_with_offsprings(offSpring_population, best_fit_parent,replacementMethod)
								.getBest_child_index());
				...
				generation_id++;
			if(arg.equalsIgnoreCase("-g") || arg.equalsIgnoreCase("-G")) {
			System.out.println("Best String: "+Replacement.replacement_of_parents_with_offsprings(offSpring_population, best_fit_parent,replacementMethod).getBest_fit_string()+
					" Worst String : "+Replacement.replacement_of_parents_with_offsprings(offSpring_population, best_fit_parent,replacementMethod).getWorst_fit_string());
	...
	/*
	 * check if average fitness doesnt improve compared to the previous 3 generations
	 * 
